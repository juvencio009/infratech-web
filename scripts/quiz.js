...
function atualizarTimerUI() {
  timer.textContent = formatTime(quizStatus.tempoRestante);
}
...
async function iniciarQuiz() {
  ...
  if (!disciplina) return alert("Disciplina não informada.");

  const data = await Loader(`../data/${disciplina}.json`);
  trackEvent('quiz_aberto', { disciplina: data.disciplina, total_questoes: data.questoes.length });
...
function renderQuiz() {
  ...
  q.opcoes.forEach((o, i) => {
    ...
    div.textContent = o;
    if (quizStatus.respostas[quizStatus.atual] === i) div.classList.add("selected");
    div.onclick = () => selecionarOpcao(i);
    containerOpcoes.appendChild(div);
  });

  backBtn.style.display = quizStatus.atual > 0 ? 'inline-block' : 'none';
  nextBtn.textContent = quizStatus.atual < quizStatus.perguntas.length - 1 ? 'Próximo' : 'Submeter';
}
...
function selecionarOpcao(index) {
  ...
  trackEvent('opcao_selecionada', { 
    disciplina: quizStatus.disciplina,
    questao: quizStatus.atual + 1,
    opcao: index
  });
}
...
function getQuestoesNaoRespondidas() {
  return quizStatus.respostas.map((r,i) => r===null ? i : null).filter(i => i!==null);
}

function tentarSubmeter(forcado=false) {
  const pendentes = getQuestoesNaoRespondidas();
  if (pendentes.length>0 && !forcado) {
    if (!confirm(`Existem ${pendentes.length} questões não respondidas. Deseja submeter mesmo assim?`)) return;
  }
  SubmeterQuiz(forcado);
}

function SubmeterQuiz(forcado=false) {
  if (quizStatus.finalizado) return;
  ...
  let historico = JSON.parse(localStorage.getItem("ProgressoQuizInfraTech_v2") || "[]");
  historico.push(progresso);
  localStorage.setItem("ProgressoQuizInfraTech_v2", JSON.stringify(historico));

  const pontuacao = quizStatus.respostas.reduce((acc, resp, idx) => resp === quizStatus.perguntas[idx].resposta ? acc+1 : acc, 0);

  trackEvent('quiz_concluido', { 
    disciplina: quizStatus.disciplina,
    pontuacao,
    total_questoes: quizStatus.perguntas.length,
    finalizadoPorTempo: forcado
  });
...
function renderAside() {
  asideEl.innerHTML = "";
  quizStatus.perguntas.forEach((_, i) => {
    const wrap = document.createElement("div");
    wrap.className="alternativaFoq";
    wrap.textContent = `Questão ${i+1}`;
    wrap.dataset.index = i;
    wrap.onclick = () => { quizStatus.atual=i; renderQuiz(); atualAside(i); };
...
function scrollAsidePara(index) {
  const item = asideEl.querySelector(`.alternativaFoq[data-index="${index}"]`);
  if(item) item.scrollIntoView({behavior:"smooth", block:"center"});
}

function atualAside(index) {
  const divs = document.querySelectorAll(".alternativaFoq");
  divs.forEach(el=>el.classList.remove("atual"));
  if(divs[index]) divs[index].classList.add("atual");
}

function markAsideSolved(index) {
  const divs = document.querySelectorAll(".alternativaFoq");
  if(divs[index]) divs[index].classList.add("solved");
}

function marcarAsideTodos() {
  quizStatus.respostas.forEach((res,i)=>{ if(res!==null) markAsideSolved(i); });
}
...
function LoadStateLocal() {
  ...
  if(Array.isArray(dados.respostas) && dados.respostas.length===quizStatus.perguntas.length)
    quizStatus.respostas = dados.respostas;
  else quizStatus.respostas = new Array(quizStatus.perguntas.length).fill(null);
  if(Number.isInteger(dados.tempoRestante) && dados.tempoRestante>0)
...
function recomecarQuiz() {
  ...
  timer.textContent=formatTime(quizStatus.tempototal);
}
...
const intros = async ()=>{
  ...
  const horas = Math.floor(dataIntro.duracao/3600);
  const minutos = Math.floor((dataIntro.duracao%3600)/60);

  duracao.innerHTML = horas>0 ? `${horas} hora${horas>1?"s":""} e ${minutos} minuto${minutos>1?"s":""}` : `${minutos} minutos`;
...
/*===================== BOTÕES =====================*/
backBtn.onclick = () => { if(quizStatus.atual>0){quizStatus.atual--;renderQuiz();atualAside(quizStatus.atual);} };
nextBtn.onclick = () => { if(quizStatus.atual<quizStatus.perguntas.length-1){quizStatus.atual++;renderQuiz();atualAside(quizStatus.atual);scrollAsidePara(quizStatus.atual);}else{tentarSubmeter();} };
retryQuizBtn.onclick = () => { localStorage.removeItem(STORAGE_KEY); location.reload(); };
goProgressBtn.onclick = () => { window.location.href="progresso.html"; };
